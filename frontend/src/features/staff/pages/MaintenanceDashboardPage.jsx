...
const MaintenanceDashboardPage = () => {
  ...
  const stats = useMemo(() => {
    return {
      ...
      pendingRepairs: maintenanceTasks.filter(t => t.status === 'PENDING').length,
      pendingCleanups: cleanTasks.filter(c => c.status !== 'READY').length
    };
  }, [gateAccesses, maintenanceTasks, cleanTasks]);

  return (
    <div id="maintenance-dashboard-container">
      <StaffLayout 
      ...
      >
        {successMessage && (
          <div className="mb-md p-md bg-emerald-50 text-emerald-800 border border-emerald-200 rounded-2xl flex items-center gap-sm shadow-sm animate-in fade-in" id="toast-success">
        ...
        {error && (
          <div className="mb-md p-md bg-rose-50 text-rose-800 border border-rose-200 rounded-2xl flex items-center gap-sm shadow-sm animate-in fade-in" id="toast-error">
