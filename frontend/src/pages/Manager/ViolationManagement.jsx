...
function ViolationManagement() {
  ...
  const handleReject = async (id) => {
    if (!window.confirm("Bạn có chắc chắn muốn từ chối biên bản này?")) return;
    try {
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: "#F5F6F8", fontFamily: "'Inter', sans-serif" }}>
      <Sidebar />
      <main style={{ marginLeft: 270, flex: 1, padding: "24px 32px", height: "100vh", overflowY: "auto", boxSizing: "border-box" }}>
        <Header 
           avatarText="MN" 
           title="Quản lý Kỷ luật (CFD)" 
           titleColor="#0A4E9B"
           bgGradient="none"
        ...
        <div style={{ background: "#FFFFFF", borderRadius: 16, boxShadow: "0 4px 20px rgba(0,0,0,0.03)", overflow: "hidden" }}>
          {loading ? (
            <div style={{ padding: 40, textAlign: "center", color: "#64748B" }}>Đang tải dữ liệu...</div>
          ) : filteredViolations.length === 0 ? (
            <div style={{ padding: 40, textAlign: "center", color: "#64748B" }}>Không có dữ liệu trong mục này</div>
          ) : (
            <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "left" }}>
              <thead>
                <tr style={{ background: "#F8FAFC", borderBottom: "1px solid #E2E8F0" }}>
                  <th style={{ padding: "16px 20px", fontSize: 13, color: "#64748B", fontWeight: 700 }}>SINH VIÊN</th>
                  <th style={{ padding: "16px 20px", fontSize: 13, color: "#64748B", fontWeight: 700 }}>ĐỊA ĐIỂM</th>
                  ...
                  <tr key={v._id} style={{ borderBottom: "1px solid #F1F5F9" }}>
                    ...
                    {activeTab === "PENDING" && (
                      <td style={{ padding: "16px 20px", textAlign: "center" }}>
                        <button
                          onClick={() => { setSelectedViolation(v); setShowModal(true); setPoints(5); }}
                          style={{ background: "#10B981", color: "white", border: "none", padding: "6px 12px", borderRadius: 6, cursor: "pointer", marginRight: 8, fontWeight: 600 }}
                        >
                          <FaCheck /> Duyệt
                        </button>
                        <button
        ...
        {showModal && (
          <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }}>
            <div style={{ background: "white", padding: 32, borderRadius: 16, width: 400, boxShadow: "0 10px 25px rgba(0,0,0,0.1)" }}>
              <h3 style={{ margin: "0 0 16px 0", color: "#0F172A" }}>Duyệt Biên Bản Vi Phạm</h3>
              <p style={{ fontSize: 14, color: "#475569", marginBottom: 20 }}>
                Sinh viên <strong>{selectedViolation?.studentId?.fullName}</strong> sẽ bị trừ điểm CFD và nhận được thông báo ngay lập tức.
              </p>
              <div style={{ marginBottom: 20 }}>
                <label style={{ display: "block", fontSize: 13, fontWeight: 700, color: "#64748B", marginBottom: 8 }}>Số điểm CFD sẽ trừ</label>
                <input
                  type="number"
                  min="1"
                  value={points}
                  onChange={(e) => setPoints(e.target.value)}
                  style={{ width: "100%", padding: 12, borderRadius: 8, border: "1px solid #CBD5E1", fontSize: 16, boxSizing: "border-box" }}
                />
              </div>
